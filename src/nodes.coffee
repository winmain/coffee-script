  ...
  compileRoot: (o) ->
    ...
    if o.goog
      ...
      for name in o.scope.getProvides()
        provideFragments.push @makeCode "goog.provide('#{name}');\n"
      for name in o.scope.getRequires()
        provideFragments.push @makeCode "goog.require('#{name}');\n"
      ...
      fragments = [].concat(provideFragments, fragments)
...
exports.Class = class Class extends Base
  ...
  ensureConstructor: (name, fullname, o) ->
    ...
    @ctorBlock = new Block [@ctor]
    if missing
      superCall = new Literal "#{fullname}.#{superClassReference(o)}.constructor.apply(this, arguments)" if @parent
      superCall = new Literal "#{@externalCtor}.apply(this, arguments)" if @externalCtor
      # In google closure mode constructor cannot return value
      if superCall and not o.goog
        ref = new Literal o.scope.freeVariable 'ref'
        @ctor.body.unshift new Assign ref, superCall
      if superCall and o.goog
        @ctor.body.unshift superCall
      @addBoundFunctions o
      if superCall and not o.goog
        @ctor.body.push ref
  ...
  compileNode: (o) ->
    ...
    if o.goog
      return [@makeCode "goog.scope(function(){\n#{@body.compile o}\n})"]
    else
  ...
  compilePatternMatch: (o) ->
    ...
    for obj, i in objects
      ...
      if not splat and obj instanceof Splat
        ...
        obj = obj.unwrap()
        val = "#{olen} <= #{vvarText}.length ? #{slice o}(#{vvarText}, #{i}"
        if rest = olen - i - 1
...
exports.Splat = class Splat extends Base
  ...
  @compileSplattedArray: (o, list, apply) ->
    ...
    if list.length is 1
      ...
      return [].concat node.makeCode("#{slice o}("), fragments, node.makeCode(")")
    args = list[index..]
    for node, i in args
      compiledNode = node.compileToFragments o, LEVEL_LIST
      args[i] = if node instanceof Splat
      then [].concat node.makeCode("#{slice o}("), compiledNode, node.makeCode(")")
...
exports.In = class In extends Base
  ...
  compileLoopTest: (o) ->
    ...
    fragments = [].concat @makeCode(indexOf(o) + "("), @array.compileToFragments(o, LEVEL_LIST),
