...
(function() {
  var ANNOTATION, ANNOTATIONS_WITH_TYPE, BOM, BOOL, CALLABLE, CODE, COFFEE_ALIASES, COFFEE_ALIAS_MAP, COFFEE_KEYWORDS, COMMENT, COMPARE, COMPOUND_ASSIGN, HEREDOC, HEREDOC_ILLEGAL, HEREDOC_INDENT, HEREGEX, HEREGEX_OMIT, IDENTIFIER, INDEXABLE, INVERSES, JSDOC_END, JSDOC_LINE, JSDOC_START, JSTOKEN, JS_FORBIDDEN, JS_KEYWORDS, LINE_BREAK, LINE_CONTINUER, LOGIC, Lexer, MATH, MULTILINER, MULTI_DENT, NOT_REGEX, NOT_SPACED_REGEX, NUMBER, OPERATOR, REGEX, RELATION, RESERVED, Rewriter, SHIFT, SIMPLESTR, STRICT_PROSCRIBED, TRAILING_SPACES, UNARY, WHITESPACE, compact, count, invertLiterate, key, last, locationDataToString, matchingParen, starts, throwSyntaxError, _ref, _ref1,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  _ref = require('./rewriter'), Rewriter = _ref.Rewriter, INVERSES = _ref.INVERSES;

  _ref1 = require('./helpers'), count = _ref1.count, starts = _ref1.starts, matchingParen = _ref1.matchingParen, compact = _ref1.compact, last = _ref1.last, invertLiterate = _ref1.invertLiterate, locationDataToString = _ref1.locationDataToString, throwSyntaxError = _ref1.throwSyntaxError;
  ...
  exports.Lexer = Lexer = (function() {
    ...
    Lexer.prototype.tokenize = function(code, opts) {
      ...
      while (this.chunk = code.slice(i)) {
        consumed = this.jsDocToken() || this.identifierToken() || this.commentToken() || this.whitespaceToken() || this.lineToken() || this.heredocToken() || this.stringToken() || this.numberToken() || this.regexToken() || this.jsToken() || this.literalToken();
    ...
    Lexer.prototype.jsDocToken = function() {
      var chunkRest, endParen, length, match, params, _ref2, _ref3;
      if (this.inJsDocComment) {
        match = JSDOC_END.exec(this.chunk);
        if (match) {
          this.token('JSDOC_END', match[0]);
          this.inJsDocComment = false;
          return match[0].length;
        }
        match = ANNOTATION.exec(this.chunk);
        if (match) {
          length = match[0].length;
          params = [];
          if (_ref2 = match[1], __indexOf.call(ANNOTATIONS_WITH_TYPE, _ref2) >= 0) {
            chunkRest = this.chunk.slice(length);
            length += /\s*/.exec(chunkRest)[0].length;
            endParen = matchingParen('{}', this.chunk, length);
            if (endParen === -1) {
              this.error("" + match[0] + " annotation requires type information in curly braces");
            }
            if (endParen === -2) {
              this.error("" + match[0] + " annotation missing closing brace }");
            }
            params.push(this.chunk.slice(length, endParen + 1));
            length = endParen + 1;
          }
          chunkRest = this.chunk.slice(length);
          length += ((_ref3 = WHITESPACE.exec(chunkRest)) != null ? _ref3[0].length : void 0) || 0;
        ...
        match = JSDOC_LINE.exec(this.chunk);
        if (match && match[0].length) {
          if (HEREDOC_ILLEGAL.test(match[0])) {
            this.error("block comment cannot contain \"*/\", starting");
          }
          this.token('JSDOC_LINE', match[0], 0, match[0].length);
          return match[0].length;
        }
      } else {
        match = JSDOC_START.exec(this.chunk);
        if (match) {
          this.token('JSDOC_START', match[0]);
          this.inJsDocComment = true;
          return match[0].length;
        }
      }
      return 0;
    ...
    Lexer.prototype.identifierToken = function() {
      var chunkRest, colon, colonOffset, endParen, forcedIdentifier, id, idLength, input, length, match, poppedToken, prev, tag, tagToken, _ref2, _ref3, _ref4;
      ...
      if (!forcedIdentifier && (__indexOf.call(JS_KEYWORDS, id) >= 0 || __indexOf.call(COFFEE_KEYWORDS, id) >= 0)) {
        ...
        if (tag === 'WHEN' && (_ref3 = this.tag(), __indexOf.call(LINE_BREAK, _ref3) >= 0)) {
          ...
          endParen = matchingParen('<>', this.chunk, length);
          if (endParen === -1) {
            this.error("Expected type delimiter '<' after cast");
          }
          if (endParen === -2) {
            this.error("Could not find matching type delimiter '>'");
          }
          this.token('CAST', this.chunk.slice(length + 1, endParen), 0, length);
          return endParen + 1;
        }
      }
